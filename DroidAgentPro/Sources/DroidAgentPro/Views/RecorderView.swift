...
struct RecorderView: View {
    ...
    private var recordingActiveContent: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Circle()
    ...
    private var recordingIdleContent: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            AppButton(
                title: "Start Recording", icon: "⏺", style: .blue,
    ...
    private var capturesGrid: some View {
                    ...
                    HStack {
                        VStack(alignment: .leading, spacing: 4) {
                            Text(session.sessionId)
                                ...
                                .foregroundColor(.primary)
                            Text("\(session.timestamp, style: .date) · \(session.events.count) events")
                                .font(.system(size: 9, design: .monospaced))
                                .foregroundColor(.textMuted)
                        }
                        Spacer()
